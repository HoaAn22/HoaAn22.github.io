...
**x** là đại diện cho dữ liệu đầu vào. \\(x_t \\) đại diện cho dữ dữ liệu đầu vào ở bước **t**. **y** là output của 1 block trong mô hình RNN. với **a** là bộ nhớ của mạng RNN \\(ht \\) là tổng hợp thông tin của lớp ẩn trước ( \\(a_t \\) -1) và input tại **t** là \\(x_t \\) 
### Công thức 
...
$$
a_t = \sigma \left( W_{xa} \cdot x_t + W_{aa} \cdot a_{t-1} + b_a \right)
$$
